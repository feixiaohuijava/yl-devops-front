    ...
    <div class="wrapper wrapper-content">
      ...
      >
        <template slot="headerRight">
          ...
          <el-form-item>
            <el-select v-model="searchHostType" placeholder="请选择主机类型">
              <el-option value="阿里云" label="阿里云"></el-option>
              <el-option value="vmware" label="vmware"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
            <el-button
        ...
        <template slot="column">
          <el-table-column label="主机ip">
            <template slot-scope="scope">
              <div class="group-name">
                ...
                <router-link
                  v-else-if="scope.row.hostType === 'vmware'"
                  :to="{
                    name: 'hostmetadata_alicloud',
                    params: { id: scope.row.id }
                  }"
                ...
                <router-link
                  v-else-if="scope.row.hostType === 'vmware物理机'"
                  :to="{
                    name: 'hostmetadata_alicloud',
                    params: { id: scope.row.id }
                  }"
                ...
                <router-link
                  v-else-if="scope.row.hostType === '物理机'"
                  :to="{
                    name: 'hostmetadata_alicloud',
                    params: { id: scope.row.id }
                  }"
          ...
          <el-table-column label="物理机">
            <template slot-scope="scope">
              <div v-if="scope.row.machine !== null">
                {{ scope.row.machine.ip }}
              </div>
            </template>
          </el-table-column>
      ...
      <el-dialog title="品牌详情" :visible.sync="editGroup.visible" width="40%">
        ...
        <span slot="footer" class="dialog-footer">
          <el-button type="primary" @click="editConfirm" v-if="buttonChange"
            >确 定</el-button
          >
          <el-button @click="cancelEdit">取 消</el-button>
        </span>
...
export default {
  ...
  methods: {
    async onSearchValue () {
      const param = {
        PrivateIpAddress: this.searchIp,
        hostType: this.searchHostType,
        HostName__contains: this.searchHostname
      }
      let res = await this.$http.selectCloudInstanceApi(param)
      this.$refs.cloudinstancelist.setData(res.results, res.count)
