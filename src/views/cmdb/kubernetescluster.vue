<template>
  <div class="main">
    <div class="wrapper wrapper-content">
      <d-table url="/api/cmdb/kubernetescluster/" ref="clusterList" v-if="!showDetail" :conditionForm="condition">
        ...
        <template slot="column">
          ...
          <el-table-column label="clusterDesription">
            <template slot-scope="scope">
              <div class="group-name">
                {{ scope.row.clusterDesription }}
              </div>
            </template>
          </el-table-column>
          <el-table-column label="apiserverUrl">
            <template slot-scope="scope">
              <div class="group-name">
                {{ scope.row.apiserverUrl }}
              </div>
            </template>
          </el-table-column>
...
<!--          </el-table-column>-->
      ...
      <el-dialog title="新建k8s集群" :visible.sync="addGroup.visible" width="40%">
        <el-form :model="addGroup.form" ref="addForm" labelWidth="100px">
          <el-form-item prop="clusterName" label="集群名" :rules="$newrules.empty">
            <el-input v-model="addGroup.form.clusterName" style="width: 95%"></el-input>
          </el-form-item>
          <el-form-item prop="clusterDesription" label="集群描述">
            <el-input v-model="addGroup.form.clusterDesription" type="textarea" :rows="2" placeholder="请输入内容" style="width: 95%"></el-input>
          </el-form-item>
          <el-form-item prop="apitoken" label="apitoken" :rules="$newrules.empty">
            <el-input v-model="addGroup.form.apitoken" type="textarea" :rows="2" placeholder="请输入内容" style="width: 95%"></el-input>
          </el-form-item>
          <el-form-item prop="apiserverUrl" label="apiserverUrl" :rules="$newrules.empty">
            <el-input v-model="addGroup.form.apiserverUrl" type="textarea" :rows="2" placeholder="请输入内容" style="width: 95%"></el-input>
          </el-form-item>
        </el-form>
  ...
  export default {
    data () {
      return {
        ...
        addGroup: {visible: false, form: {clusterName: '', clusterDesription: '', apitoken: '', apiserverUrl: ''}},
        showDetail: false,
        detailId: '',
        detailName: '',
        activeName: '',
    ...
    methods: {
      ...
      addConfirm () {
        this.$refs.addForm.validate(async v => {
          if (v) {
            const param = {
              'clusterName': this.addGroup.form.clusterName,
              'clusterDesription': this.addGroup.form.clusterDesription,
              'apitoken': this.addGroup.form.apitoken,
              'apiserverUrl': this.addGroup.form.apiserverUrl
            }
            let res = await this.$http.addCluster(param)
            this.addGroup.visible = false
            this.$refs.clusterList.getData()
          }
        })
      },
      edit (item) {
      ...
      async getClusterList (key) {
        let res = await this.$http.get('/api/cmdb/kubernetescluster/')
        this.$refs.clusterList.setData(res.results, res.count)
      },
      getPerm () {
