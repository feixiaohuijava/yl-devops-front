<template>
  <div class="main">
    <div class="wrapper wrapper-content">
      <d-table url="/api/webhookmanager/robotviewset/" ref="brandList" v-if="!showDetail" :conditionForm="condition">
        ...
        <template slot="column">
          ...
          <el-table-column label="机器人名" width="200px">
            <template slot-scope="scope">
              {{ scope.row.robot_name }}
            </template>
          </el-table-column>
          <el-table-column label="机器人地址" width="400px">
            <template slot-scope="scope">
              {{ scope.row.robot_url }}
            </template>
          </el-table-column>
          <el-table-column label="channel" width="100px">
            <template slot-scope="scope">
          ...
          <el-table-column label="机器人类型" width="200px">
            <template slot-scope="scope">
              {{ scope.row.robot_type }}
            </template>
          </el-table-column>
          <el-table-column label="二级机器人标签">
            <template slot-scope="scope" v-if=" scope.row.robot_url_label.length == 0 ">
              {{ }}
            </template>
            <template slot-scope="scope" v-if=" scope.row.robot_url_label.length != 0 ">
              <el-tag v-for="tag in scope.row.robot_url_label" :key="tag" :type="success">{{tag}}</el-tag>
            </template>
          </el-table-column>
