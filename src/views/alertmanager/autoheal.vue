<template>
  <div class="main">
    <div class="wrapper wrapper-content">
      <d-table url="/api/webhookmanager/autohealpackageviewset/" ref="brandList" v-if="!showDetail" :conditionForm="condition">
        <template slot="headerRight">
          <el-form-item>
            <el-button type="primary">
              <router-link  :to="{name:'autoheallogmemory'}">自愈记录</router-link>
            </el-button>
        ...
        <template slot="column">
          ...
          <el-table-column label="自愈套餐名" width="200px">
            <template slot-scope="scope">
              <div v-if="scope.row.healName ==='基础监控内存清除'">
                {{ scope.row.healName }}
              </div>
              <div v-else-if="scope.row.healName === 'rabbitmq死信队列'">
                <router-link  :to="{name:'autoheallograbbitmq'}">{{ scope.row.healName }}</router-link>
              </div>
              <div v-else>
                {{ scope.row.healName }}
              </div>
          ...
          <el-table-column label="自愈操作" width="400px">
            <template slot-scope="scope">
              {{ scope.row.healOperation }}
            </template>
          </el-table-column>
          ...
          <el-table-column label="updateTime" width="200px">
            <template slot-scope="scope">
