<template>
  <div class="main">
    <div class="wrapper wrapper-content">
      <d-table url="/api/dbaudit/polardbslowlogviewset/" ref="brandList" v-if="!showDetail" :conditionForm="condition">
        ...
        <template slot="column">
          ...
          <el-table-column label="实例" width="200px">
            <template slot-scope="scope">
              {{ scope.row.DBClusterId.DBClusterId }}
            </template>
          </el-table-column>
          <el-table-column label="DBClusterDescription">
            <template slot-scope="scope">
              {{ scope.row.DBClusterId.DBClusterDescription }}
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <router-link class="document-btn" :to="{name:'db_polardbSlowLog_detail', params: {topTenSlowLogs: scope.row.topTenSlowLogs}}">慢SQL前十名</router-link>
            </template>
          </el-table-column>
        </template>
      </d-table>
