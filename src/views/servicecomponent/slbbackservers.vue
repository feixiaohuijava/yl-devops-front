<template>
  <div class="main">
    <div class="wrapper wrapper-content">
      <d-table url="/api/cmdb/aliyunslbbs/" ref="bsslblist" v-if="!showDetail" :conditionForm="condition">
        <template slot="headerRight">
          <el-form-item>
            <el-input placeholder="请输入nginxIp" v-model="nginxIp"></el-input>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="searchBsslb()">查询</el-button>
          </el-form-item>
          <el-form-item>
        ...
        <template slot="column">
          ...
          <el-table-column label="slb后端关联Nginx">
            <template slot-scope="scope">
              {{ scope.row.ServerId}}
            </template>
          </el-table-column>
          <el-table-column label="nginxIp">
            <template slot-scope="scope">
              {{ scope.row.nginxIp}}
            </template>
          </el-table-column>
          <el-table-column label="slb地址">
            <template slot-scope="scope">
              {{ scope.row.cloudSlb.LoadBalancerId}}
              <br>
              {{ scope.row.cloudSlb.LoadBalancerName}}
            </template>
          </el-table-column>
...
<script>
    export default {
        ...
        methods: {
            async searchBsslb() {
                const param = {nginxIp: this.nginxIp}
                let res = await this.$http.getBsslbApi(param)
                this.$refs.bsslblist.setData(res.results, res.count)
