<template>
  <div class="main">
    <div class="wrapper wrapper-content">
      <d-table url="/api/dbaudit/mongodbviewset/" ref="brandList" v-if="!showDetail" :conditionForm="condition">
        ...
        <template slot="column">
          ...
          <el-table-column label="DBInstanceId" width="200px">
            <template slot-scope="scope">
              {{ scope.row.DBInstanceId }}
            </template>
          </el-table-column>
          <el-table-column label="DBInstanceType">
            <template slot-scope="scope">
              {{ scope.row.DBInstanceType }}
            </template>
          </el-table-column>
          ...
          <el-table-column label="DBInstanceClass">
            <template slot-scope="scope">
              {{ scope.row.DBInstanceClass }}
            </template>
          </el-table-column>
          <el-table-column label="ShardList" width="260px">
            <template slot-scope="scope">
              <div v-if="scope.row.ShardList !== null">
                {{ scope.row.ShardList.ShardAttribute }}
              </div>
            </template>
          </el-table-column>
          <el-table-column label="MongosList" width="200px">
            <template slot-scope="scope">
              <div v-if="scope.row.MongosList !== null">
                {{ scope.row.MongosList.MongosAttribute }}
              </div>
            </template>
          </el-table-column>
          <el-table-column label="DBInstanceDescription">
            <template slot-scope="scope">
              {{ scope.row.DBInstanceDescription }}
            </template>
          </el-table-column>
          <el-table-column label="d_CurrentKernelVersion">
            <template slot-scope="scope">
              {{ scope.row.d_CurrentKernelVersion}}
            </template>
          </el-table-column>
        </template>
      </d-table>
